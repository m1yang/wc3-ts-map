...
import { getElapsedTime } from "../globals/gametime";
...
export class SyncRequest {
  ...
  private send(packet: SyncOutgoingPacket) {
    ...
    if (this.from === MapPlayer.fromLocal()) {
      DzSyncData(prefix, packet.toString());
    }
  }
  ...
  private static init() {
    ...
    for (let i = 0; i < bj_MAX_PLAYER_SLOTS; i++) {
      ...
      if (
        ...
        this.eventTrigger.registerSyncEvent(SYNC_PREFIX, false);
        this.eventTrigger.registerSyncEvent(SYNC_PREFIX_CHUNK, false);
      }
    }
  ...
  private static onSync() {
    const syncPrefix = DzGetTriggerSyncPrefix();
